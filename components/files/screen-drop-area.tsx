      ...
      if (files && files.length > 0) {
        ...
        try {
          ...
          if (result.success) {
            ...
            setUploadError(result.error ? result.error : "出错了...")
          }
        } catch (error) {
          console.error("Upload error:", error)
          setUploadError(error instanceof Error ? error.message : "出错了...")
        } finally {
        ...
        >
          <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">
              {transactionId ? "拖放文件以添加到交易" : "拖放文件以上传"}
            </h3>
            <p className="text-gray-600 dark:text-gray-400">拖放到屏幕任意位置</p>
          </div>
        </div>
      )}
      ...
      {uploadError && (
        <div className="fixed inset-0 bg-opacity-20 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">上传错误</h3>
