...
import { categorizeRestaurantExpense } from "@/lib/poc-categorize"
...
export async function analyzeFileAction(
  ...
  if (results.success && results.data?.output) {
    ...
    const ruleCategory = categorizeRestaurantExpense({
      merchant: output.merchant,
      text: output.text,
      name: output.name,
    })

    output.categoryCode = ruleCategory.categoryCode

    if (!output.note) {
      output.note = `规则分类：${ruleCategory.reason}`
    }
  }
