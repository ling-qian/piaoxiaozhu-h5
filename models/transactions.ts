...
export const getTransactionsByMonth = cache(async (userId: string, month: string) => {
  const monthStart = new Date(`${month}-01`)
  const nextMonth = monthStart.getMonth() + 1
  const nextYear = monthStart.getFullYear() + (nextMonth > 11 ? 1 : 0)
  const adjustedMonth = nextMonth > 11 ? 0 : nextMonth
  const nextMonthStart = new Date(nextYear, adjustedMonth, 1)
