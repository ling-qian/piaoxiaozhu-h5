      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            我们在数据库中发现了一笔与该新记录的日期、金额和商户匹配的已有交易。您希望如何处理？
          </DialogDescription>
        </DialogHeader>
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex justify-between border-b pb-1">
                ...
                <span className="font-medium">
                  {existingData.issuedAt ? format(new Date(existingData.issuedAt), "yyyy-MM-dd") : "无"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">商户</span>
                <span className="font-medium truncate ml-4 text-right">{existingData.merchant || "未知"}</span>
              </div>
              <div className="flex justify-between border-b pb-1">
          ...
          <Card className="border-blue-200 bg-blue-50/30 dark:border-blue-900/50 dark:bg-blue-900/10">
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">日期</span>
                <span className="font-medium">
                  {newData.issuedAt ? format(new Date(newData.issuedAt as unknown as string), "yyyy-MM-dd") : "无"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">商户</span>
                <span className="font-medium truncate ml-4 text-right">
                  {(newData.merchant as string) || "未知"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">金额</span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">类型</span>
                <span className="capitalize">{(newData.type as string) || "无"}</span>
              </div>
            </CardContent>
          </Card>
        </div>
