    ...
    <>
      {csvData.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-2 h-full min-h-[400px]">
          <p className="text-muted-foreground">上传 CSV 文件以导入交易记录</p>
          <div className="flex flex-row gap-5 mt-8">
            <div>
              ...
              <Button type="button" onClick={() => document.getElementById("csv-file")?.click()}>
                {isParsing ? "解析中..." : <Upload className="mr-2" />} 从 CSV 导入
              </Button>
            </div>
          </div>
        ...
        <div>
          <header className="flex flex-wrap items-center justify-between gap-2 mb-8">
            <h2 className="flex flex-row gap-3 md:gap-5">
              <span className="text-3xl font-bold tracking-tight">从 CSV 导入 {csvData.length} 条记录</span>
            </h2>
            <div className="flex gap-2">
              <Button onClick={handleSave} disabled={isSaving}>
                {isSaving ? (
                  <>
                    <Loader2 className="animate-spin" /> 导入中...
                  </>
                ) : (
                  <>
                    <Play /> 导入 {csvData.length} 条交易
                  </>
                )}
              </Button>
            </div>
          ...
          {csvData.length > MAX_PREVIEW_ROWS && (
            <p className="text-muted-foreground mt-4">还有 {csvData.length - MAX_PREVIEW_ROWS} 条记录...</p>
          )}
        </div>
      )}
    </>
