...
function validateInvoiceType(type: string | null, invoiceNo: string | null, invoiceCode: string | null): string | null {
 ...
 if (invoiceCode) {
 // 数电票/电子票的发票代码应为12位数字
 if (type === "vat_special_electronic" || type === "vat_normal_electronic" || type === "electronic") {
 if (!/^[0-9]{12}$/.test(invoiceCode)) {
 return null; // 代码无效，强制降级为未知类型，前端可提示"发票代码疑似伪造"
 }
 }
 // 纸质票的发票代码应为10位数字
 if (type === "vat_special" || type === "vat_normal" || type === "machine_printed") {
 if (!/^[0-9]{10}$/.test(invoiceCode)) {
 return null; // 代码无效，强制降级为未知类型
 }
 }
 }
